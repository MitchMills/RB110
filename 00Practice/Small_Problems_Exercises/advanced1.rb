...
PROBLEM 6:56
input: array
  - contains three subarrays
    - each subarray contains three elements
output: new array
  - transpose of input array
    - rows become columns
      - each subarray in a particular index position
        - contains all elements at that index postion from each subarray

EXAMPLES

DATA STRUCTURES
needs:
  - way to access each subarray
  - way to access each element in each subarray
  - new array with subarrays
...
ALGORITHM
- create an empty array with three empty subarrays
- for each subarray in the input array
  - for each element in that subarray, with its index
    - add the element at that index to the new subarray with that index
...
def transpose(matrix)
  matrix.map.with_index do |subarray, row_index|
    subarray.map.with_index do |element, column_index|
