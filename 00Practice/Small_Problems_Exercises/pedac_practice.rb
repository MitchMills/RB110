...
PROBLEM __________
Given a sequence of positive integers and a target positive integer, find the smallest sub-sequence of integers that when summed are greater than or equal to the target integer

Input:  - array
          - each element in input array is an integer
          - all integers are positive

        - integer
          - is positive
          - represents target number

Output: - integer
          - length of the shortest subarray of contiguous integers that add up to a number greater than or equal to the target number
          - a subarray can be only one element
          - if the sum of all the integers in the input array is less than the target number, output should be 0
...
- if the sum of all integers is less than the target number: 0
- if the largest number in the array is greater than or equal to the target number: 1
- if the sum of all integers equals the target number: array length

DATA STRUCTURES __________
inputs: - array # --> [2, 3, 1, 2, 4, 3]
        - integer # --> 7

all subarrays: # --> [2], [2, 3], [2, 3, 1], [2, 3, 1, 2] etc

successful subarrays: [2, 3, 1, 2], [3, 1, 2, 4], [1, 2, 4], [2, 4, 3], [4, 3]
  - maybe store these in a nested array
  - maybe just store lengths in a flat array
  - or use a variable `shortest_length` that gets updated when a shorter subarray is found

  - or look at all length 1 subarrays, then length 2, etc, and return length as soon as a sum >= target number is found
    - don't need to store all possible arrays and/or successful arrays

output: integer # --> 2 (length of shortest successful subarray)

ALGORITHM __________
- check all length 1 subarrays, then length 2, etc, up to length of input array
  - iterate from length 1 up to length of array # [1, 2, 3, 4, 5] --> (1..5)

    - start at index 0, then index 1, etc
      - iterate from index 0 up to index (array length - subarray length)
            length 1: start indexes (0..4) --> [1], [2], [3], [4], [5]
            length 2: start indexes (0..3) --> [1,2], [2,3], [3,4], [4,5]
            length 3: start indexes (0..2) --> [1,2,3], [2,3,4], [3,4,5]
        - get the subarray at that start index with that length
        - if sum of that subarray is >= target number
          - return current length of subarrays

  - return 0 if no subarray found after all iterations
=end

# def min_subarray_length(array, target)
#   (1..array.length).each do |subarray_length|
#     (0..(array.length - subarray_length)).each do |start_index|
#       if array[start_index, subarray_length].sum >= target
